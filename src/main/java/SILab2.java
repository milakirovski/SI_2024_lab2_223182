...
public class SILab2 {
    public static boolean checkCart(List<Item> allItems, int payment){
        ...
        for (int i = 0; i < allItems.size(); i++){ //2
            ...
            if (item.getBarcode() != null){     // 4
                String allowed = "0123456789";
                char chars[] = item.getBarcode().toCharArray();
                for (int j = 0; j < item.getBarcode().length(); j++){       // 5
                    char c = item.getBarcode().charAt(j);
                    if (allowed.indexOf(c) == -1){                  // 6
                        throw new RuntimeException("Invalid character in item barcode!");
                    }
                }
                if (item.getDiscount() > 0){                    // 7
                    sum += item.getPrice()*item.getDiscount();
                }
                else {
                    sum += item.getPrice(); // TODO: NEMA POPUST !
                }
            }
            else {
                throw new RuntimeException("No barcode!");
            }
            if (item.getPrice() > 300 && item.getDiscount() > 0 && item.getBarcode().charAt(0) == '0'){     // 8
                sum -= 30;
            }
        }
        if (sum <= payment){                                                                                 // 9
